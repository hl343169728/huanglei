...
<html>
  <head>
    ...
    <title>My JSP 'memo_update.jsp' starting page</title>
			...
			$.ajax({
				...
				success:function(result){
					if(result == "1"){
						...
						$.messager.confirm('提示信息','备忘录的结束时间必须在开始时间之后，请重新核对！','error');
  ...
  <body>
    <div id="p" class="easyui-panel" title="修改备忘录" icon="icon-save"
		collapsible="true"  maximizable="true" 
		style="width: 1000px; height: 500px; padding: 10px; background: #fafafa;">
			<form id="myform" action="MemoAction!updateMemo" method="post" onsubmit="return check();">
				<input name="id" type="hidden" value="${memo.id }"/>
				<table>
					<tr>
						<td>内容：</td>
						<td>
							<textarea name="memo.content" rows="5" cols="60">${memo.content}</textarea>
						</td>
					</tr>
					<tr>
						<td>开始时间：</td>
						<td>
							<input id="startTime" name="memo.startTime" value="${memo.startTime }"/>
						</td>
					</tr>
					<tr>
						<td>结束时间：</td>
						<td>
							<input id="endTime" name="memo.endTime" value="${memo.endTime}"/>
						</td>
					</tr>
					<tr>
						<td colspan="2">
							<input id="btn" class="easyui-linkbutton" iconCls="icon-ok" type="submit" value="修改备忘录"/>
						</td>
					</tr>
				</table>
			</form>
