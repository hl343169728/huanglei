	...
	<script type="text/javascript">
			$(function(){
				...
				$('#t_in').datagrid({
						...
						url: 'MessageAction!inList?flag=1',	 
						remoteSort: false ,
						...
						columns:[[
							{field:'title',		title:'标题'  , 		width:80 , align:'center'  , formatter: function( value , record ,index){
								  if(record.readState == '未读'){
									  return '<span style=color:red; ondblclick=read('+record.messageId+','+record.id+') >'+value+'</span>'; 
								  }else {
									  return '<span style=color:black; ondblclick=read('+record.messageId+','+record.id+') >'+value+'</span>'; 
								  }
		...
		//去读列表 
		function read(messageid , receiverid){
			  window.location = 'MessageAction!readMessage?messageId=' + messageid+"&receiverid=" +receiverid ; 
		}
		...
		function addDust(id){
			$.messager.confirm('提示信息' , '确认加入垃圾箱?' , function(r){
				...
				if(r){
					window.location = 'MessageAction!addDust?receiverid=' + id ;
				}
			});
		}
