	...
	<script type="text/javascript">
		$(function(){
			...
			$('#t_realStu').datagrid({
					...
					url: 'StudentAction!getRealStudentList?flag=4', 	//发送Ajax异步请求 从服务器端取数据 （从远程请求数据的地址）  
					...
					columns:[[
						...
						{field:'fee',           	title:'应缴总学费' ,			width:150, align:'center'  },
						{field:'feeUpdateReason',   title:'修改总学费的原因' ,	width:150, align:'center'  },
						{field:'description',   	title:'备注' ,				width:200, align:'center'  },
						{field:'operation',   		title:'操作' ,				width:100, align:'center'  ,
							formatter : function(value , record , index){
							return "<a href='javascript:void(0);' onclick=preDropOut("+record.id+")>退学</a>" ;  	     									
							}
						} 
					]]
			});
			displayMsg($('#t_realStu')); 
		});
		//分页 
		...
		//退学函数
		function preDropOut(id){
			$.messager.confirm('信息提示','确认退学吗?',function(r){
				if(r){
					window.location = "StudentAction!preDropOut?id="+id;
				}
			});
		};
	</script>
